...
public class UserActivity extends BaseActivity {
	...
	@Bind(R.id.user_sex) TextView userSex;

	@Bind(R.id.user_loginout_btn) Button userLoginout;

	@Bind(R.id.user_change_btn) Button userChange;

	@Bind(R.id.user_touxiang) CircleImageView userTouxiang;
	...
	private void initViews() {
		...
		dialogPlus = DialogPlus.newDialog(UserActivity.this)
			...
			.setOnItemClickListener(new OnItemClickListener() {
				@Override public void onItemClick(DialogPlus dialog, Object item, View view, final int position) {
	...
	@OnClick({ R.id.user_change_btn, R.id.user_loginout_btn }) public void userClick(View view) {
		switch (view.getId()) {
			...
			case R.id.user_loginout_btn:
				if (loginType == 1) {
					...
					new MaterialDialog.Builder(UserActivity.this).title("登出账号？")
						.content("点击确定登出您的账号。")
						.positiveText("确定")
						.negativeText("取消")
